...
export const getGeneralValidationSchema = (required: boolean) =>
  yup.object().shape({
    ...
    manager: stringSchema(labels.manager, {
      required,
    }),
    design: stringSchema(labels.design, {
      required,
    }),
    build_yard: stringSchema(labels.build_yard, {
      required,
    }),
    rig_status: stringSchema(labels.rig_status, {
      nullable: true,
      required,
    }),
...
export const getCapacitiesValidationSchema = (required: boolean) =>
  yup.object().shape({
    ...
    dual_derrick: booleanSchema(labels.dual_derrick, {
      required,
    }),
    ...
    riser_on_board_outfitted: numberSchema(labels.riser_on_board_outfitted, {
      min: 0,
      max: 20000,
      required,
    }),
    riser_storage_inside_hull: booleanSchema(labels.riser_storage_inside_hull, {
      required,
    }),
    split_funnels_free_stern_deck: booleanSchema(
      labels.split_funnels_free_stern_deck,
      {
        required,
      },
    ),
...
export const getOperationValidationSchema = (required: boolean) =>
  yup.object().shape({
    tripsaver: booleanSchema(labels.tripsaver, {
      required,
    }),
    offline_stand_building: booleanSchema(labels.offline_stand_building, {
      required,
    }),
    auto_pipe_handling: booleanSchema(labels.auto_pipe_handling, {
      required,
    }),
    dual_activity: booleanSchema(labels.dual_activity, {
      required,
    }),
    drilltronic: booleanSchema(labels.drilltronic, {
      required,
    }),
    ...
    automatic_tripping: booleanSchema(labels.automatic_tripping, {
      required,
    }),
    closed_bus: booleanSchema(labels.closed_bus, {
      required,
    }),
    scr: booleanSchema(labels.scr, {
      required,
    }),
    hybrid: booleanSchema(labels.hybrid, {
      required,
    }),
    hvac_heat_recovery: booleanSchema(labels.hvac_heat_recovery, {
      required,
    }),
    ...
    hpu_optimization: booleanSchema(labels.hpu_optimization, {
      required,
    }),
